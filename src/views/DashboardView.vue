      ...
      <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-10 z-10 shrink-0 min-w-[850px]">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 tracking-tight">
            ...
            <span v-if="activeTab === 'analytics'">Financial Analytics</span>
            <span v-if="activeTab === 'balances'">Monthly Budgets</span>
            <span v-if="activeTab === 'goals'">Savings Goals</span>
            <span v-if="activeTab === 'bills'">Subscriptions & Bills</span>
            <span v-if="activeTab === 'transactions'">All Transactions</span>
            <span v-if="activeTab === 'settings'">Profile Settings</span>
          </h1>
        ...
        <div class="flex items-center space-x-6">
          <div class="flex items-center pl-6 border-l border-slate-200">
            <div class="w-10 h-10 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-700 font-bold text-lg mr-3 shadow-sm overflow-hidden">
              <img v-if="user?.profile_picture" :src="'https://expense-manager-api.42web.io/backend/uploads/' + user.profile_picture" class="w-full h-full object-cover">
      ...
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <form @submit.prevent="submitTransaction" class="p-6 space-y-5">
          ...
          <div>
            ...
            <input v-model="txForm.note" type="text" placeholder="What was this for?" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium text-slate-700">
          </div>
    ...
    <div v-if="isBudgetModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-blue-50">
          <h3 class="text-lg font-bold text-blue-800">Set Budget: {{ activeBudget?.category_name }}</h3>
    ...
    <div v-if="isBillModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-indigo-50">
          <h3 class="text-lg font-bold text-indigo-800">Add Subscription / Bill</h3>
        ...
        <form @submit.prevent="submitBill" class="p-6 space-y-5">
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Bill Name</label>
            <input v-model="billForm.name" type="text" required placeholder="e.g. Netflix, Rent" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium text-slate-700">
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Amount</label>
            ...
            <div>
              ...
              <input v-model="billForm.due_date" type="number" min="1" max="31" required placeholder="e.g. 15" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-900">
            </div>
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Link to Category</label>
...
const fetchProfile = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_user_details.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
...
const updateProfile = async () => {
  ...
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/update_profile.php', { 
        method: 'POST', 
...
const uploadAvatar = async (event) => {
  ...
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/upload_profile_picture.php', { method: 'POST', body: formData })
    const data = await res.json()
    if (data.status === 'success') {
...
const removeAvatar = () => {
  requireConfirmation('Remove Picture', 'Are you sure you want to remove your profile picture?', async () => {
    try {
      const res = await fetch('https://expense-manager-api.42web.io/backend/user/remove_profile_picture.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
      const data = await res.json()
      if (data.status === 'success') {
...
const updatePassword = async () => {
  ...
  isSubmitting.value = true
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/change_password.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, current_password: passForm.current, new_password: passForm.new }) })
    const data = await res.json()
    if (data.status === 'success') {
...
const fetchBills = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_bills.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
    const data = await res.json()
...
const submitBill = async () => {
  isSubmitting.value = true
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/add_bill.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, ...billForm }) })
    if ((await res.json()).status === 'success') { fetchBills(); isBillModalOpen.value = false; }
  } catch (e) { alert('Server error.') } finally { isSubmitting.value = false }
}
const payBill = async (id) => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/pay_bill.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, bill_id: id, month: selectedMonth.value }) })
...
const deleteBill = (id) => {
  requireConfirmation('Delete Subscription', 'Are you sure you want to delete this bill?', async () => {
    try {
      const res = await fetch('https://expense-manager-api.42web.io/backend/user/delete_bill.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, bill_id: id }) })
...
const fetchCategories = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_categories.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
...
const fetchDashboardStats = async () => {
  try {
    const statsRes = await fetch('https://expense-manager-api.42web.io/backend/user/get_overview_stats.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
    const statsData = await statsRes.json()
    if (statsData.status !== 'error') stats.value = statsData

    const transRes = await fetch('https://expense-manager-api.42web.io/backend/user/get_recent_transactions.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
...
const fetchAllTransactions = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_all_transactions.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
    const data = await res.json()
...
const fetchAnalytics = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_expense_breakdown.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, month: selectedMonth.value }) })
    const data = await res.json()
...
const fetchBudgets = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_budgets.php', { 
...
const submitBudget = async () => {
  isSubmitting.value = true
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/set_budget.php', {
...
const fetchGoals = async () => {
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/get_goals.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id }) })
    const data = await res.json()
...
const submitGoal = async () => {
  isSubmitting.value = true
  const endpoint = isGoalEditMode.value ? 'https://expense-manager-api.42web.io/backend/user/update_goal.php' : 'https://expense-manager-api.42web.io/backend/user/add_goal.php'
...
const deleteGoal = (id) => {
  requireConfirmation('Delete Goal', 'Are you sure you want to delete this savings goal?', async () => {
    try {
      const res = await fetch('https://expense-manager-api.42web.io/backend/user/delete_goal.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, goal_id: id }) })
...
const submitContribution = async () => {
  isSubmitting.value = true
  try {
    const res = await fetch('https://expense-manager-api.42web.io/backend/user/add_contribution.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, goal_id: activeGoal.value.id, amount: contribForm.amount }) })
    if ((await res.json()).status === 'success') { 
      fetchGoals(); 
      fetchDashboardStats();
      fetchAllTransactions();
...
const submitTransaction = async () => {
  isSubmitting.value = true
  const endpoint = isEditMode.value ? 'https://expense-manager-api.42web.io/backend/user/update_transaction.php' : 'https://expense-manager-api.42web.io/backend/user/add_transaction.php'
...
const deleteTransaction = (id) => {
  requireConfirmation('Delete Transaction', 'Are you sure you want to delete this transaction?', async () => {
    try {
      const res = await fetch('https://expense-manager-api.42web.io/backend/user/delete_transaction.php', { method: 'POST', body: JSON.stringify({ user_id: user.value.id, transaction_id: id }) })
      if ((await res.json()).status === 'success') {
        await fetchDashboardStats()
        if (activeTab.value === 'transactions') await fetchAllTransactions()
        if (activeTab.value === 'balances') await fetchBudgets()
